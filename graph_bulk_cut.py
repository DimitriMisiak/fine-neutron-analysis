...
for atupl in ax_tuples:
    ...
    ax.plot(
            ...
            # marker=',',
            marker='o', markersize=1,
            alpha=0.5, zorder=10, color='coral',
            label='Trapped Events'
    )
    
    if xind == 'A':
        ax.axvline(-thresh_a, color='k')
        ax.axvline(+thresh_a, color='k')

    if xind == 'C':
        ax.axvline(-thresh_c, color='k')
        ax.axvline(+thresh_c, color='k')

    if yind == 'A':
        ax.axhline(-thresh_a, color='k')
        ax.axhline(+thresh_a, color='k')

    if yind == 'C':
        ax.axhline(-thresh_c, color='k')
        ax.axhline(+thresh_c, color='k')        
...
fig.savefig(
    '/home/misiak/Analysis/NEUTRON/thesis_plots/bulk_cut.png',
    dpi=600
)